...
import { Mutation, Publisher, PubSub, Resolver, Subscription, UseMiddleware } from 'type-graphql';
...
export default class AdminDashboardSubscriber {
  ...
  async startAdminSubscriber(@PubSub('ADMIN_DASHBOARD') publisher: Publisher<any>) {
    await publisher(await backendData());
    return true;
  }
